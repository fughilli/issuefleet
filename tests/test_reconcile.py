...
from issuefleet import MARKER_PREFIX, config, gitops
...
class ReconcileTest(unittest.TestCase):
    ...
    def test_restart_survives_a_git_error_staging_the_exclude(self):
        """add_worktree_exclude shells out to git, so a stale worktree admin
        dir surfaces as GitError, not OSError — it must not escape past
        restarts += 1 either."""
        w = self.claim_one()
        (Path(w.worktree) / ".claude-container-overlay").unlink()
        def boom(repo, path, pattern):
            raise gitops.GitError("rev-parse --git-common-dir failed")
        self.git.add_worktree_exclude = boom
        self.runner.dead.add(w.tmux_session)
        self.rec.tick()
